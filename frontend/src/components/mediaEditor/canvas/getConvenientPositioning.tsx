...
import {snapToViewport, getCurrentImageDimensions, getCurrentImageRatio} from '../utils';
...
export default function getConvenientPositioning({
  ...
  // Get image dimensions from current file or use default
  const imageDimensions = getCurrentImageDimensions(editorState, mediaState);
  ...
  // Use file-specific image ratio instead of global
  const [cropWidth, cropHeight] = snapToViewport(fileImageRatio, cropOffset().width, cropOffset().height);
