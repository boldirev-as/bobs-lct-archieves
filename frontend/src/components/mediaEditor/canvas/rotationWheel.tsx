...
export default function RotationWheel() {
  ...
  onMount(() => {
    ...
    const swipeHandler = new SwipeHandler({
      ...
      onStart() {
        ...
        initialScale = currentFile?.transform?.scale || mediaState.scale;
  ...
  const onSwipe = withCurrentOwner(() => {
    ...
    const currentFile = editorState.targetFile;
    const currentRotation = currentFile?.transform?.rotation || mediaState.rotation;
    const targetRotation = currentRotation - rotationDiff;

    const r = [Math.cos(rotationDiff), Math.sin(rotationDiff)];

    const currentTranslation = currentFile?.transform?.translation || mediaState.translation;
    const targetTranslation: NumberPair = [
      currentTranslation[0] * r[0] + currentTranslation[1] * r[1],
      currentTranslation[1] * r[0] - currentTranslation[0] * r[1]
    ];
    ...
    // Update file-specific transform if we have a target file
    ...
    if (targetFile) {
      ...
      targetFile.transform.rotation = targetRotation;
      targetFile.transform.translation = targetTranslation;
      if(additionalScale > 1) {
        targetFile.transform.scale = initialScale * additionalScale;
      }
    } else {
      // Fallback to global state if no target file
