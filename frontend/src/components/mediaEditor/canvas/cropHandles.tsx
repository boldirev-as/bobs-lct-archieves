...
import {withCurrentOwner, getCurrentImageRatio, getCurrentTransform, getCurrentCanvasSize, getCurrentFile} from '../utils';
import {NumberPair, UploadedFile} from '../types';
...
export default function CropHandles(props: CropHandlesProps) {
  const {editorState, mediaState} = useMediaEditorContext();
  
  // Use file from props or fallback to current file
  const getFile = () => props.file || getCurrentFile(editorState, mediaState);
  ...
  const getNewLeftTopAndSize = () => {
    // Use file-specific image ratio
    const fileImageRatio = getCurrentImageRatio(editorState, mediaState);
    const [width, height] = snapToViewport(fileImageRatio, cropOffset().width, cropOffset().height);
  ...
  onMount(() => {
    ...
    const resizeSwipeHandlers = multipliers.map(({el, left, top}) => {
      return new SwipeHandler({
        ...
        onStart() {
          // Use file-specific transforms
          const transform = getCurrentTransform(editorState, mediaState);
          initialScale = transform.scale;
          initialTranslation = transform.translation;
        ...
        onSwipe(xDiff, yDiff, e) {
          ...
          const fixed = targetFile?.fixedImageRatioKey;
          // Use file-specific image ratio
          const imageDimensions = targetFile?.imageDimensions || [800, 600];
          let ratio = imageDimensions[0] / imageDimensions[1];
          if(left < 0) {
            ratio = -ratio;
          }
          if(top < 0) {
          ...
          if(additionalScale > 1) {
            const currentFileForScale = getFile();
            if (currentFileForScale?.transform) {
              currentFileForScale.transform.scale = initialScale * additionalScale;
            }
          }
          ...
          const r = [Math.sin(initialRotation), Math.cos(initialRotation)];

          boundDiff = [boundDiff[0] * r[1] - boundDiff[1] * r[0], boundDiff[1] * r[1] + boundDiff[0] * r[0]];

          const currentFileForTranslation = getFile();
          if (currentFileForTranslation?.transform) {
            currentFileForTranslation.transform.translation = [initialTranslation[0] - boundDiff[0] / 2, initialTranslation[1] - boundDiff[1] / 2];
          }
        },
        onReset() {
          ...
          const initialTranslation = transform.translation;

          const targetScale = initialScale * upScale;
          const targetTranslation = [
            upScale * (initialTranslation[0] + -diff()[0] * left * 0.5),
            upScale * (initialTranslation[1] + -diff()[1] * top * 0.5)
          ];
          ...
          animateValue(
            ...
            200,
            (progress) => {
              const currentFileForAnimation = getFile();
              if (currentFileForAnimation?.transform) {
                currentFileForAnimation.transform.scale = lerp(initialScale, targetScale, progress);
                currentFileForAnimation.transform.translation = lerpArray(initialTranslation, targetTranslation, progress) as NumberPair;
              }
            },
            {
    ...
    const translationSwipeHandler = new SwipeHandler({
      ...
      onSwipe(xDiff, yDiff, e) {
        ...
        const currentTransform = getCurrentTransform(editorState, mediaState);
        const {cropMinX, cropMaxX, cropMinY, cropMaxY, imageMinX, imageMaxX, imageMinY, imageMaxY} =
          getConvenientPositioning({
        ...
        const r = [Math.sin(currentTransform.rotation), Math.cos(currentTransform.rotation)];

        boundDiff = [boundDiff[0] * r[1] - boundDiff[1] * r[0], boundDiff[1] * r[1] + boundDiff[0] * r[0]];

        const resistance = 4;
        const currentFileForDrag = getFile();
        if (currentFileForDrag?.transform) {
          currentFileForDrag.transform.translation = [
      ...
      onReset() {
        ...
        if (!currentFile?.transform) return;
        
        const prevTranslation = currentFile.transform.translation;
        animateValue(
          prevTranslation,
          [prevTranslation[0] - boundDiff[0], prevTranslation[1] - boundDiff[1]] as const,
          120,
          (value) => {
            if (currentFile?.transform) {
              currentFile.transform.translation = value;
            }
          },
          {
            onEnd: () => editorState.isMoving = false
  ...
  const croppedSizeFull = () => {
    const canvasSize = getCurrentCanvasSize(editorState, mediaState);
    const [cw, ch] = canvasSize;
    let [w, h] = [cw, ch];
    const ratio = getCurrentImageRatio(editorState, mediaState);
