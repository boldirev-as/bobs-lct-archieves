...
export default function BrushTab() {
  ...
  return (
    <>
      ...
      />
      <Space amount="16px" />
      <div class="media-editor__label">Tool</div>
      {brushButton('Pen', <PenBrush />, 'pen')}
      {brushButton('Arrow', <ArrowBrush />, 'arrow')}
      {brushButton('Brush', <MarkerBrush />, 'brush')}
      {brushButton('Neon', <NeonBrush />, 'neon')}
      {mediaType === 'image' && brushButton('Blur', <BlurBrush />, 'blur')}
      {brushButton('Eraser', <EraserBrush />, 'eraser')}
    </>
  );
}
