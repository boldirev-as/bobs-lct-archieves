...
import {FontInfo, FontKey, NumberPair, ResizableLayer, UploadedFile} from './types';
import {HistoryItem, MediaEditorState, EditingMediaState} from './context';
import {IS_FIREFOX} from '../../environment/userAgent';

export const log = logger('Media editor');

/**
 * Get current file from editor state, ensuring we get the actual file from mediaState
 */
export function getCurrentFile(editorState: MediaEditorState, mediaState: EditingMediaState): UploadedFile | undefined {
  if (!editorState.targetFile) return undefined;
  
  // Find the actual file in mediaState.uploadedFiles by ID to ensure we have the latest version
  const actualFile = mediaState.uploadedFiles.find(f => f.id === editorState.targetFile?.id);
  return actualFile || editorState.targetFile; // Fallback to targetFile if not found
}

/**
 * Get current file's image dimensions with fallback
 */
export function getCurrentImageDimensions(editorState: MediaEditorState, mediaState: EditingMediaState): NumberPair {
  const currentFile = getCurrentFile(editorState, mediaState);
  return currentFile?.imageDimensions || [800, 600];
}

/**
 * Get current file's image ratio with fallback
 */
export function getCurrentImageRatio(editorState: MediaEditorState, mediaState: EditingMediaState): number {
  const dimensions = getCurrentImageDimensions(editorState, mediaState);
  return dimensions[0] / dimensions[1];
}

/**
 * Get current file's transform with fallback
 */
export function getCurrentTransform(editorState: MediaEditorState, mediaState: EditingMediaState) {
  const currentFile = getCurrentFile(editorState, mediaState);
  return currentFile?.transform || {
...
/**
 * Get current file's canvas size with fallback
 */
export function getCurrentCanvasSize(editorState: MediaEditorState, mediaState: EditingMediaState): NumberPair {
  const currentFile = getCurrentFile(editorState, mediaState);
  return currentFile?.canvasSize || [800, 600];
}

/**
 * Get current file's rendering payload
 */
export function getCurrentRenderingPayload(editorState: MediaEditorState, mediaState: EditingMediaState) {
  const currentFile = getCurrentFile(editorState, mediaState);
  return currentFile?.renderingPayload;
}
