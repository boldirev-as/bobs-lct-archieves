...
import {FinalTransform, NumberPair} from '../types';
...
export default function getScaledLayersAndLines(
  ...
  const resizableLayers = currentFile.resizableLayers || [];
  const brushDrawnLines = currentFile.brushDrawnLines || [];
  const pixelRatio = editorState.pixelRatio;
