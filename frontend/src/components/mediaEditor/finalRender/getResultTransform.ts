import {FinalTransform} from '../types';
...
export default function getResultTransform({
  ...
  const canvasSize = currentFile.canvasSize;
  const {scale, translation, rotation, flip} = currentFile.transform;
  const currentImageRatio = mediaState.currentImageRatio;
