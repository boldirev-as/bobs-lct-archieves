...
def vec_regions(heatmap: torch.Tensor, cls_map: Dict, scale: float, **kwargs) -> Dict[str, List[Region]]:
    ...
    for reg_type, regs in regions.items():
        regions[reg_type] = [Region(id=f'_{uuid.uuid4()}', boundary=x, tags={'type': [{'type': reg_type}]}) for x in
                             scale_regions(regs, scale)]
    return regions
...
def segment(im: PIL.Image.Image,
    ...
    if model is None:
        ...
        print(resources.files(__package__).joinpath('blla.mlmodel'))
        model = vgsl.TorchVGSLModel.load_model(resources.files(__package__).joinpath('blla.mlmodel'))
    ...
    for line in lines:
        ...
        blls.append(
            BaselineLine(id=f'_{uuid.uuid4()}', baseline=line['baseline'], boundary=line['boundary'], tags=line['tags'],
                         regions=line_regs))
